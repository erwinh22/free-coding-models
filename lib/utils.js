...
// GLM 5 (and similar models) occasionally produce invalid JSON in tool_call
// function.arguments, e.g. trailing periods, missing closing braces/brackets,
// or unclosed strings.  These pure functions attempt best-effort repair so that
// JSON.parse succeeds.

/**
 * Attempt to repair malformed JSON text produced by LLMs.
 * Returns the original text if it already parses, or a repaired version.
 * If repair is not possible, returns the original text unchanged.
 */
export function repairJson(text) {
  ...
  // 1. Strip trailing period (GLM 5 signature bug: `"value".`)
  if (s.endsWith('.') && !s.endsWith('..')) {
    s = s.slice(0, -1)
    try { JSON.parse(s); return s } catch { /* continue */ }
  }

  // 2. Close unclosed strings — odd number of unescaped double-quotes
  ...
  // Strip trailing commas before we close (e.g. `{"a":1,}` → `{"a":1}`)
  s = s.replace(/,\s*$/, '')
  if (brackets > 0) s += ']'.repeat(brackets)
  if (braces > 0) s += '}'.repeat(braces)
  try { JSON.parse(s); return s } catch { /* continue */ }

  // 4. Attempt to strip everything after the last balanced top-level brace
  // (handles trailing garbage like `{"a":1}extra`)
...
/**
 * Walk an OpenAI-format chat-completion response object and repair any
 * malformed JSON in tool_call function.arguments fields.
 * Mutates the object in-place and returns it.
 */
export function repairToolCallArgs(data) {
  ...
  for (const choice of choices) {
    ...
    for (const tc of toolCalls) {
      ...
      // Handle arguments sent as object instead of string
      if (tc.function.arguments && typeof tc.function.arguments === 'object') {
        tc.function.arguments = JSON.stringify(tc.function.arguments)
      }
      // Handle missing/empty arguments — default to empty object
