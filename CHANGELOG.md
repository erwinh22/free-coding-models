...
- **"Stab" column renamed to "Stability"** — column header widened from 6 to 11 characters; header text now reads `Stability` with the `B` sort-key letter highlighted in bold white (dim `Sta` + bold `b` + dim `ility`).
